...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		// 处理跨域请求中的Preflight请求
//		http.csrf().ignoringAntMatchers("/eureka/**").disable().authorizeRequests().requestMatchers(CorsUtils::isPreFlightRequest).permitAll();
		http.csrf().disable().authorizeRequests().requestMatchers(CorsUtils::isPreFlightRequest).permitAll();
		// http.sessionManagement().maximumSessions(1).expiredUrl("/login.html");//这个是控制
		// 只有一个人登陆 待验证
		/** 基于token 屏蔽session */
		http.sessionManagement().sessionCreationPolicy(SessionCreationPolicy.STATELESS);

		/** 静态资源不拦截 */
		http.authorizeRequests().antMatchers("/", "/es/buss/exportOutHomeCsv","/buss/app/**","/app/**", // /app/**就不会拦截app开头的所有的连接了 因为这个地方是配置静态的拦截器的
				"/*.html", //
				"/css/**", //
				"/js/**", //
				"/fonts/**", //
