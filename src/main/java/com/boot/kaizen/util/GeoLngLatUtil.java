...
/**
 * 地理经纬度操作工具类 参考链接：https://www.cnblogs.com/leejuan/p/5552460.html
 * 
 * @author weichengz
 * @date 2020年4月9日 上午11:29:03
 */
public class GeoLngLatUtil {
	 ...
	 * @return 两点距离，单位km
	 */
	public static double GetDistanceOne(double lon1, double lat1, double lon2, double lat2) {
		double r1 = rad(lat1);
		double r2 = rad(lon1);
		double a = rad(lat2);
		double b = rad(lon2);
		double s = Math.acos(Math.cos(r1) * Math.cos(a) * Math.cos(r2 - b) + Math.sin(r1) * Math.sin(a)) * EARTH_RADIUS;
		return s;
	}
	 ...
	 * @return 两点距离，单位km
	 */
	public static double GetDistanceThree(double lon1, double lat1, double lon2, double lat2) {
		...
		double d = Math.pow((x1 - x2), 2) + Math.pow((y1 - y2), 2) + Math.pow((z1 - z2), 2);
		// // 余弦定理求夹角
		// double theta = Math.acos((2 - d) / 2);

		d = Math.pow(EARTH_RADIUS, 2) * d;
		// //余弦定理求夹角
		double theta = Math.acos((2 * Math.pow(EARTH_RADIUS, 2) - d) / (2 * Math.pow(EARTH_RADIUS, 2)));
